...
public class GameScene: SKScene, SKPhysicsContactDelegate
{
    ...
    {
        player.position = GCPoint(x:self.frame.midX, y:player.size.height/2 + 10)
        addChild(player)
    }
    ...
    func newGame() -> Void
    {
        ...
        let transitionType = SKTransitionflipHorizontal(withDuration: 0.5)
    ...
    override public func didMove(to view: SKView) -> Void
    {
        ...
        let starField = SKEmitterNode(fileNamed: "Star Field")
    ...
    {
        player.physiBody?.velocity = CGVector(dx: accelerationX * 600)
    }
    ...
    public func didBegin(_ contact: SKPhysicsContact) -> Void
    {
            ...
            (secondBody.categoryBitMask & CollisionCategories.PlayerBullet != 0))
        {
            ...
            let thenvader = firstBody.node as! Invader
            let newInvaderRow = theInvader - 1
            let newInvaderCol = theInvader.invaderCol
            if(newInvaderRow >= 1)
            {
                self.enumeraterChildNodes(withName: "invader")
                {
                    node, stop in
